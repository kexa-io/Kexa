...
export function propertyToSend(rule: Rules, objectContent: any, isSms: boolean = false): string {
    if (isSms) {
        ...
        if (rule?.objectName === 'KexaAzure.users') {
            return `Id: <a href="https://portal.azure.com/#view/Microsoft_AAD_UsersAndTenants/UserProfileMenuBlade/~/overview/userId/${escapeHtml(objectContent?.id)}">${escapeHtml(objectContent?.id)}</a>`;
        } else if (rule?.objectName === 'KexaAzure.conditionnalAccess') {
            return `Id: <a href="https://portal.azure.com/#view/Microsoft_AAD_ConditionalAccess/ConditionalAccessBlade/~/Overview">${escapeHtml(objectContent?.id)}</a>`;
        } else if (rule?.objectName === 'KexaAzure.servicePrincipals') {
            return `Id: <a href="https://portal.azure.com/#view/Microsoft_AAD_IAM/ManagedAppMenuBlade/~/Overview/objectId/${escapeHtml(objectContent?.id)}/appId/1e70cd27-4707-4589-8ec5-9bd20c472a46/${escapeHtml(objectContent?.appId)}">${escapeHtml(objectContent?.displayName)}</a>`;
        } else if (rule?.objectName === 'KexaAzure.applications') {
            return `Id: <a href="https://portal.azure.com/#view/Microsoft_AAD_RegisteredApps/ApplicationMenuBlade/~/Overview/appId/${escapeHtml(objectContent?.appId)}">${escapeHtml(objectContent?.displayName)}</a>`;
        } else {
            return `Id: <a href="https://portal.azure.com/#@/resource/${escapeHtml(objectContent?.id)}">${escapeHtml(objectContent?.id)}</a>`;
        }
    }
}