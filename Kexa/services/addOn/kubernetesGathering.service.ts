...
export async function kubernetesListing(isPathKubeFile: boolean): Promise<any> {
    ...
    const namespacePromises = namespaces.body.items.map(async (item: any) => {
        const promises = [
            ...
            collectDeployment(k8sApiCore, item.metadata.name),
            collectReplicaset(k8sApiCore, item.metadata.name),
            collectStatefulset(k8sApiCore, item.metadata.name),
            collectDaemonset(k8sApiCore, item.metadata.name),
            collectJob(k8sApiCore, item.metadata.name),
            collectCronjob(k8sApiCore, item.metadata.name),
            collectIngress(k8sApiCore, item.metadata.name),
            ...
            collectRole(k8sApiCore, item.metadata.name),
            collectRolebinding(k8sApiCore, item.metadata.name),
            collectClusterrole(k8sApiCore, item.metadata.name),
            collectClusterrolebinding(k8sApiCore, item.metadata.name),
            collectStorageclass(k8sApiCore, item.metadata.name),
            collectNetworkpolicy(k8sApiCore, item.metadata.name),
            ...
            collectApiservice(k8sApiCore, item.metadata.name),
            collectControllerrevision(k8sApiCore, item.metadata.name),
            collectLease(k8sApiCore, item.metadata.name),
            collectCertificate(k8sApiCore, item.metadata.name),
            collectCertificateSigningRequest(k8sApiCore, item.metadata.name),
...
async function collectIngress(k8sNetworkingV1Api: any, namespace: string): Promise<any> {
    try {
        const ingress = await k8sNetworkingV1Api.listNamespacedIngress();
        return ingress;
    } catch (e) {
        logger.debug(e);
        return null;
