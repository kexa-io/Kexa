 ...
 * fast pre-check on a literal config URL and on every address a hostname
 * actually resolves to (see httpGathering.service.ts's createSsrfSafeLookup,
 * which is what actually closes the DNS-rebinding/redirect gaps below). */
 ...
 * pointing at one, both bypass it. httpGathering.service.ts's
 * createSsrfSafeLookup() is what actually closes those gaps, at the
 * socket-connect level, by calling isPrivateIp() on resolved addresses. */
